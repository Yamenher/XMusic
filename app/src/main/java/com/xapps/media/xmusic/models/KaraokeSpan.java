    ...
    public void draw(
        ...
        if (progress > -0.99f) {
            ...
            if (glowAlpha > 0.01f && progress >= 0.0f && progress < 1.0f) {
                ...
                float edgeFactor = Math.min(1f, Math.max(0f, progress));
                float radius = 12f + (23f * edgeFactor); // starts small, grows

                paint.setShadowLayer(
                        radius,
                        0,
                        0,
                        (originalColor & 0x00FFFFFF) | ((int) (finalShadowAlpha * 255) << 24));
            } else {
                paint.clearShadowLayer();
            }
