...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle _savedInstanceState) {
    ...
    initializeLogic();
    setupCallbacks();
    mlfa = (MusicListFragmentActivity) getSupportFragmentManager().findFragmentById(
        R.id.fragmentsContainer);
    ViewCompat.setOnApplyWindowInsetsListener(
        binding.miniPlayerBottomSheet, (v, insets) -> { return WindowInsetsCompat.CONSUMED; });
    executor.execute(() -> {
  ...
  private void initialize(Bundle _savedInstanceState) {
    ...
    binding.miniPlayerBottomSheet.setBackground(
        ContextCompat.getDrawable(this, R.drawable.rounded_corners_bottom_sheet));
    ...
    binding.nextButton.setOnClickListener(v -> {
      ...
      if (seekAllowed) {
        currentPosition++;
        _setSong(currentPosition,
            SongsMap.get(currentPosition).get("thumbnail") == null
                ? placeholderUri
                : SongsMap.get(currentPosition).get("thumbnail").toString(),
            Uri.parse("file://" + SongsMap.get(currentPosition).get("path").toString()));
        seekAllowed = false;
        seekController.postDelayed(() -> { seekAllowed = true; }, 150);
      }
    });
  ...
  private void initializeLogic() {
    ...
    bottomSheetColor = MaterialColors.getColor(
        this, com.google.android.material.R.attr.colorSurfaceContainer, 0xff000000);
    ...
    binding.songSeekbar.addOnChangeListener((slider, progress, isManual) -> {
      if (isManual) {
        binding.currentDurationText.setText(
            SongMetadataHelper.millisecondsToDuration((int) progress));
      }
    });
    ...
    binding.songSeekbar.addOnSliderTouchListener(new Slider.OnSliderTouchListener() {
      ...
      public void onStopTrackingTouch(Slider slider) {
        new Handler(Looper.getMainLooper()).postDelayed(() -> { seekbarFree = true; }, 101);
        Intent playIntent = new Intent(c, PlayerService.class);
    ...
    bottomSheetBehavior.addBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
      ...
      public void onStateChanged(@NonNull View bottomSheet, int newState) {
        if (newState == BottomSheetBehavior.STATE_DRAGGING) {
          ...
          if (newState == BottomSheetBehavior.STATE_HIDDEN) {
            ...
            isBsInvisible = true;
            XUtils.increaseMargins(MusicListFragmentActivity.fab, 0, 0, 0,
                -(binding.coversPager.getHeight()
                    + binding.miniPlayerBottomSheet.getPaddingTop() * 2));
          } else {
            isBsInvisible = false;
          }
          if (!isBNVHidden())
      ...
      public void onSlide(@NonNull View bottomSheet, float slideOffset) {
        ...
        if (0f < slideOffset) {
          ...
          if (!isBNVHidden())
            binding.bottomNavigation.setTranslationY(
                binding.bottomNavigation.getHeight() * slideOffset * 2.5f);
          ...
          if (slideOffset >= 0.5f) {
            ...
            tmpColor =
                XUtils.interpolateColor(bottomSheetColor, playerSurface, slideOffset * 2 - 1f);
    ...
    binding.bottomNavigation.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
      if (!isRun) {
        ...
        int bottomHeight = binding.coversPager.getHeight() + binding.bottomNavigation.getHeight()
            + binding.musicProgress.getHeight() + XUtils.getMargin(binding.musicProgress, "top");
      }
      isRun = true;
    });
  }
  ...
  public void addFragmentWithTransition(androidx.fragment.app.Fragment fragment) {
    ...
    androidx.fragment.app.Fragment current =
        getSupportFragmentManager().findFragmentById(R.id.fragmentsContainer);
  ...
  public void _setSong(final int _position, final String _coverPath, final Uri _fileUri) {
    ...
    if (isBsInvisible) {
      XUtils.increaseMarginsSmoothly(MusicListFragmentActivity.fab, 0, 0, 0,
          (binding.coversPager.getHeight() + binding.miniPlayerBottomSheet.getPaddingTop() * 2),
          200L);
      bottomSheetBehavior.setState(BottomSheetBehavior.STATE_COLLAPSED);
    }
    MusicListFragmentActivity frag =
        (MusicListFragmentActivity) getSupportFragmentManager().findFragmentById(
            R.id.fragmentsContainer);
    if (frag != null) {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if ("PLAYER_PROGRESS".equals(action) && intent.hasExtra("progress")) {
        ...
        if (currentPosition < binding.songSeekbar.getValueTo() && seekbarFree) {
          ...
          binding.currentDurationText.setText(
              SongMetadataHelper.millisecondsToDuration(currentPosition));
        }
  ...
  public void HideBNV(boolean hide) {
    if (hide) {
      binding.bottomNavigation.animate()
          ...
          .setDuration(100)
          .withEndAction(() -> binding.bottomMixer.removeView(binding.bottomNavigation))
          .start();
      ...
      bottomSheetBehavior.setPeekHeight(
          Math.round(bsh + binding.bottomNavigation.getPaddingBottom()), true);
      binding.bottomNavigation.animate()
          .alpha(1f)
          .translationY(0)
          .setDuration(100)
          .withStartAction(() -> binding.bottomMixer.addView(binding.bottomNavigation))
          .start();
      if (SongsMap.size() != 0) {
        MusicListFragmentActivity.fab.show();
      }
  ...
  public void updateColors() {
    if (ColorPaletteUtils.lightColors != null || ColorPaletteUtils.darkColors != null) {
      binding.toggleView.setShapeColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("primary")
              : ColorPaletteUtils.lightColors.get("onPrimary"));
      playerSurface = XUtils.isDarkMode(c) ? ColorPaletteUtils.darkColors.get("surface")
                                           : ColorPaletteUtils.lightColors.get("surface");
      binding.toggleView.setIconColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onPrimary")
              : ColorPaletteUtils.lightColors.get("primary"));
      binding.nextButton.getBackground().setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onTertiary")
              : ColorPaletteUtils.lightColors.get("onTertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.favoriteButton.getBackground().setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onTertiary")
              : ColorPaletteUtils.lightColors.get("onTertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.saveButton.getBackground().setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onTertiary")
              : ColorPaletteUtils.lightColors.get("onTertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.previousButton.getBackground().setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onTertiary")
              : ColorPaletteUtils.lightColors.get("onTertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.nextButton.setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("tertiary")
              : ColorPaletteUtils.lightColors.get("tertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.favoriteButton.setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("tertiary")
              : ColorPaletteUtils.lightColors.get("tertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.saveButton.setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("tertiary")
              : ColorPaletteUtils.lightColors.get("tertiary"),
          PorterDuff.Mode.SRC_IN);
      binding.previousButton.setColorFilter(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("tertiary")
              : ColorPaletteUtils.lightColors.get("tertiary"),
          PorterDuff.Mode.SRC_IN);
      Slider slider = binding.songSeekbar;
      slider.setTrackInactiveTintList(ColorStateList.valueOf(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("outline")
              : ColorPaletteUtils.lightColors.get("outline")));
      slider.setTrackActiveTintList(ColorStateList.valueOf(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("primary")
              : ColorPaletteUtils.lightColors.get("primary")));
      slider.setThumbTintList(ColorStateList.valueOf(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("primary")
              : ColorPaletteUtils.lightColors.get("primary")));
      slider.setHaloTintList(ColorStateList.valueOf(Color.TRANSPARENT));
      binding.artistBigTitle.setTextColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onSurfaceContainer")
              : ColorPaletteUtils.lightColors.get("onSurfaceContainer"));
      binding.songBigTitle.setTextColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onSurface")
              : ColorPaletteUtils.lightColors.get("onSurface"));
      binding.currentDurationText.setTextColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onSurfaceContainer")
              : ColorPaletteUtils.lightColors.get("onSurfaceContainer"));
      binding.totalDurationText.setTextColor(XUtils.isDarkMode(c)
              ? ColorPaletteUtils.darkColors.get("onSurfaceContainer")
              : ColorPaletteUtils.lightColors.get("onSurfaceContainer"));
      Drawable background = binding.miniPlayerBottomSheet.getBackground();
  ...
  public void updateTexts(int pos) {
    if (currentMap.size() > 0) {
      binding.totalDurationText.setText(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos)
              .get("duration")
              .toString());
      binding.artistBigTitle.setText(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos).get("author").toString());
      binding.songBigTitle.setText(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos).get("title").toString());
      binding.currentSongTitle.setText(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos).get("title").toString());
      binding.currentSongArtist.setText(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos).get("author").toString());
  ...
  public void updateMaxValue(int pos) {
    if (currentMap.size() > 0) {
      int max = Integer.parseInt(
          currentMap.get(pos == -1 ? PlayerService.currentPosition : pos).get("total").toString());
  ...
  public void setupCallbacks() {
    callback = new OnBackPressedCallback(false) {
      ...
      public void handleOnBackProgressed(BackEventCompat backEvent) {
        ...
        binding.miniPlayerBottomSheet.setTranslationY(
            (binding.miniPlayerBottomSheet.getHeight() * 0.05f) * backEvent.getProgress());
      }
