...
public class SerializationUtils {
    public static void saveToFile(Context context, ArrayList<HashMap<String, Object>> data, String fileName) throws IOException {
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(
            ...
            android.util.Log.d("Serialization", String.format("Save time: %.3f ms", (endTime - startTime) / 1_000_000.0));
        }
    }
    ...
    public static ArrayList<HashMap<String, Object>> readFromFile(Context context, String fileName) {
        ...
        if (file.exists() && file.length() > 0) {
            try (ObjectInputStream ois = new ObjectInputStream(
                ...
                long endTime = System.nanoTime();
                android.util.Log.d("Serialization", String.format("Read time: %.3f ms", (endTime - startTime) / 1_000_000.0));
                return result;
            } catch (IOException | ClassNotFoundException e) {
                android.util.Log.e("Serialization", "Error reading file: " + e.getMessage());
                return null;
            }
        } else {
            android.util.Log.d("Serialization", "File not found or empty: " + fileName);
            return null;
        }
    }
}
