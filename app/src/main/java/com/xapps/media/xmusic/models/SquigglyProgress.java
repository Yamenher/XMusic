...
import com.xapps.media.xmusic.utils.CalculationUtils;


public class SquigglyProgress extends Drawable {
	
	@VisibleForTesting
	static final int DISABLED_ALPHA = 77;
	...
	public void draw(Canvas canvas) {
		...
		float waveProgressPx = totalWidth * (!transitionEnabled || progress > matchedWaveEndpoint ? progress
		: CalculationUtils.lerp(minWaveEndpoint, matchedWaveEndpoint,
	...
	private void updateColors(int tintColor, int alpha) {
		...
		linePaint.setColor(CalculationUtils.setAlphaComponent(
		tintColor, 
		(int) (DISABLED_ALPHA * (alpha / 255f))
		));
	}
}
